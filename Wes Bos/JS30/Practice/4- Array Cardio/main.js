 // Get your shorts on - this is an array workout!
    ...
    const totalAge = inventors.reduce((total, inv) => total + (inv.passed - inv.year), 0);
    ...
    /*const groupsContainer = document.querySelector('.mw-category');
    const allLinks = Array.from(groupsContainer.querySelectorAll('a'));
    const linksTextWithDe = allLinks.map( link => link.innerText)
                                    .filter(text => text.includes('de'));*/

    // 7. sort Exercise
    // Sort the people alphabetically by last name
    const sortedyLastName = people.sort((a, b) => {
      const [aFirst, aLast] = a.split(', ')
      const [bFirst, bLast] = b.split(', ');
      return (aLast < bLast)? -1 : 1;
    });
    console.log(sortedyLastName);

    // 8. Reduce Exercise
    // Sum up the instances of each of these
    const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck' ];
    const numOfInstances = data.reduce((obj, element) =>{
      if (!obj[element])
          obj[element] = 0;
      obj[element]++;
      return obj;
